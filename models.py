...
def create_staff(department_id, full_name, email, phone, hashed_password):
    db = get_db()
    try:
        with db.cursor() as cur:
            cur.execute(
                '''INSERT INTO staff (department_id, full_name, email, phone, password, role)
                   VALUES (%s,%s,%s,%s,%s,'field_officer') RETURNING id''',
                (department_id, full_name, email, phone or None, hashed_password)
            )
        ...
        with db.cursor() as cur:
            cur.execute(
                '''INSERT INTO complaints
                   (user_id,title,description,category_id,subcategory,lat,lng,image_path)
        ...
        with db.cursor() as cur:
            cur.execute(
                '''SELECT c.*, u.full_name as submitted_by
                   ...
                   WHERE c.id=%s''', (complaint_id,))
            row = cur.fetchone()
            return dict(row) if row else None
    finally:
        db.close()

def get_all_complaints(status=None, category_id=None, limit=500, offset=0):
    db = get_db()
    try:
        with db.cursor() as cur:
            where, params = [], []
            ...
            if category_id:
                where.append('c.category_id=%s'); params.append(category_id)
            w = ('WHERE ' + ' AND '.join(where)) if where else ''
            cur.execute(
                f'''SELECT c.*, u.full_name as submitted_by
                    FROM complaints c JOIN users u ON c.user_id=u.id
                    {w} ORDER BY c.submitted_at DESC
                    LIMIT %s OFFSET %s''',
                params + [limit, offset]
            )
            return [dict(r) for r in cur.fetchall()]
...
def get_complaints_by_user(user_id):
    ...
    try:
        with db.cursor() as cur:
            cur.execute(
                'SELECT * FROM complaints WHERE user_id=%s ORDER BY submitted_at DESC',
                (user_id,))
            return [dict(r) for r in cur.fetchall()]
    finally:
        db.close()
...
def update_complaint_status(complaint_id, new_status, changed_by_staff_id=None):
    ...
    try:
        with db.cursor() as cur:
            ...
            extra = ''
            if new_status == 'resolved': extra = ', resolved_at=NOW()'
            elif new_status == 'closed': extra = ', closed_at=NOW()'
            cur.execute(
                f'UPDATE complaints SET status=%s{extra} WHERE id=%s',
                (new_status, complaint_id)
            )
            cur.execute(
                '''INSERT INTO complaint_history
                   (complaint_id,changed_by_staff,field_changed,old_value,new_value)
                   VALUES (%s,%s,'status',%s,%s)''',
                (complaint_id, changed_by_staff_id, old_status, new_status)
            )
        db.commit()
        ...
        with db.cursor() as cur:
            ...
            cur.execute(
                '''SELECT category_id, COUNT(*) as count
                   FROM complaints GROUP BY category_id ORDER BY count DESC'''
            )
            stats['by_category'] = [dict(r) for r in cur.fetchall()]
            for k in ['total','open','in_progress','resolved','closed','today']:
                stats[k] = int(stats[k] or 0)
            if stats.get('avg_resolution_hours'):
                stats['avg_resolution_hours'] = float(stats['avg_resolution_hours'])
            return stats
    finally:
        db.close()
